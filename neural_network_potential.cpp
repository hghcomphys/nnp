...
void NeuralNetworkPotential::readSetupFiles(const std::string& directory)
{
    ...
    for (auto &element: elements) {
            ...
            // read weights
            char filename[32];
            sprintf(filename, "weights.%3.3d.data", Atom::getAtomicNumber(element.c_str()));
...
double NeuralNetworkPotential::caculateTotalEnergy(AtomicStructure& structure) 
{
    ...
    Atom **listOfAtoms = structure.getListOfAtoms();
    const int numberOfAtoms = structure.getNumberOfAtoms();

    for(int i=0; i<numberOfAtoms; i++)
         totalEnergy += calculateEnergy(structure, listOfAtoms[i]->index);
