...
std::vector<std::vector<double>> ACSF::calculate(AtomicStructure &structure)
{
    ...
    Atom **listOfAtomsForCentralElement = structure.getListOfAtomsForElement(centralElement.c_str());
    const int numberOfAtomsForCentralElement = structure.getNumberOfAtomsForElement(centralElement.c_str());
...
std::vector<double> ACSF::calculate(AtomicStructure &structure, int atomIndex)
{
    ...
    for (int n=0; n<n_2b; n++) 
    {
        Atom **listOfAtomsForNeighborElement = structure.getListOfAtomsForElement(listOfTwoBodyNeighborElement[n].c_str());
        const int numberOfAtomsForNeighborElement = structure.getNumberOfAtomsForElement(listOfTwoBodyNeighborElement[n].c_str());
        ...
        for(int j=0; j<numberOfAtomsForNeighborElement; j++) 
        {
            ...
            Distance& distance_ij = structure.getTableOfDistances()[atom_i->index][atom_j->index];
            if ( distance_ij.dr > rcGlobal ) continue;
    ...
    for (int n=0; n<n_3b; n++) 
    {
        ...
        Atom **listOfAtomsForNeighborElement1 = structure.getListOfAtomsForElement(listOfThreeBodyNeighborElement1[n].c_str());
        const int numberOfAtomsForNeighborElement1 = structure.getNumberOfAtomsForElement(listOfThreeBodyNeighborElement1[n].c_str());
     
        Atom **listOfAtomsForNeighborElement2 = structure.getListOfAtomsForElement(listOfThreeBodyNeighborElement2[n].c_str());
        const int numberOfAtomsForNeighborElement2 = structure.getNumberOfAtomsForElement(listOfThreeBodyNeighborElement2[n].c_str());
        ...
        {        
            ...
            for(int k=0; k<numberOfAtomsForNeighborElement2; k++) 
            {
                ...
                Distance& distance_ik = structure.getTableOfDistances()[atom_i->index][atom_k->index];
                if ( distance_ik.dr > rcGlobal ) continue;

                // const double rjk = structure.distance(atom_j, atom_k);
                Distance& distance_jk = structure.getTableOfDistances()[atom_j->index][atom_k->index];
                if ( distance_jk.dr > rcGlobal ) continue;
