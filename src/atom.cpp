/*
  atom.cpp: This file is part of Free Molecular Dynamics
...
/* ----------------------------------------------------------------------
   setup for Atom
------------------------------------------------------------------------- */
Atom::Atom(int index, const std::string &element, const double position[3]) : index(index), element(element), fx(0), fy(0), fz(0), charge(0), energy(0)
{
    setPosition(position);
}

Atom::Atom(int index, const std::string &element, const double position[3], const double force[3], double charge, double energy) : index(index), element(element), charge(charge), energy(energy)
{
    setPosition(position);
    setForce(force);
}
