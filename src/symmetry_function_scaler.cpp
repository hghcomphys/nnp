/*
  symmetry_function_scaler.cpp: This file is part of Free Molecular Dynamics
...
/* ----------------------------------------------------------------------
   setup for scaler
------------------------------------------------------------------------- */
Scaler::Scaler(double min, double max, double mean, double sigma) : min(min), max(max), mean(mean), sigma(sigma), smin(0.0), smax(1.0)
{
    setScalingFactor();
}
...
------------------------------------------------------------------------- */
SymmetryFunctionsScaler::SymmetryFunctionsScaler() : numberOfWarnings(0), maxNumberOfWarnings(100) {}

void SymmetryFunctionsScaler::addScaler(const Scaler &newScaler)
{
    listOfScalers.push_back(newScaler);
}

int SymmetryFunctionsScaler::getNumberOfScalers()
{
    return listOfScalers.size();
}

void SymmetryFunctionsScaler::readScaling(const std::string &filename, int elementIndex)
{
    std::string line;
...
void SymmetryFunctionsScaler::scale(double *descriptorValues, int descriptorSize)
{
...
std::vector<double> SymmetryFunctionsScaler::getScalingFactors()
{
...
void SymmetryFunctionsScaler::setMaxNumberOfWarnings(double number)
{
    maxNumberOfWarnings = number;
}

int SymmetryFunctionsScaler::getMaxNumberOfWarnings()
{
